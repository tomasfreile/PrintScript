...
import ast.Node
import parser.semantic.NumberTypeDeclaration
import token.Token
...
class DeclarationParse : Parse {
        ...
        return if (hasBinaryOperation(tokenList.subList(rightParenIndex + 1, tokenList.size)))
            { // maybe is (3 + 5) * 3
                ASTBinaryNode(
                    ...
                    tokenList[rightParenIndex + 1], // * the operator
                )
            } else {
            parse(tokenList.subList(1, tokenList.size))
    ...
    private fun constructSplitNode(
        ...
        return ASTBinaryNode(
            ...
            tokenList[splitIndex], // Papa node
        )
    }
