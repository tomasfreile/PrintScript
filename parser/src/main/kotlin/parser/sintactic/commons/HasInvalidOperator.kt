...
class HasInvalidOperator: SintacticChecker { //SOLO REVISA QUE SE MANTENGA EL FORMATO LITERAL OPERADOR LITERAL EN EL CONTENIDO (LOL RULE)
    ...
    private fun checkStructure(tokenList: List<Token>, from: Int, to: Int): Boolean{ //format: LITERAL OPERATOR LITERAL...
        ...
        while(index < content.size){
            ...
            index += when{
                ...
                isAnOperator(content[index]) ->{ //At least index should be index = 1
                    if(isFormatted(content, index)) 1
                    else return true
                }
                else -> return true
            }
    ...
    private fun isFormatted(content: List<Token>, index: Int): Boolean{  // L O L jsjs
